...
#
# 파이프라인:
#  [1] 1차: 하단 30% 크롭 프레임 추출 + OCR + 필터
#  [2] 자막 밀도 < 5/분 → full-frame 폴백 재스캔
#  [3] Claude 1차 맞춤법 + needsRescan 플래그
#  [4] 의심 자막(플래그 + needsRescan)만 full-frame 재스캔
...
# ================================================================
# [1/6] 전체 프레임 추출 (자막 분류기가 스타일·위치 무관 자막 탐지)
# ================================================================
echo ""
echo "[1/6] 프레임 추출 (full-frame, 0.5초 간격)"
...
# ================================================================
# [2/6] OCR 배치 + Python 필터 (수명 + 의심 플래그)
# ================================================================
echo ""
echo "[2/6] Vision OCR + 필터"
...
# ================================================================
# [3/6] Claude 1차 맞춤법 검사 (needsRescan 플래그 포함)
# ================================================================
echo ""
echo "[3/6] Claude 1차 맞춤법"
...
needsRescan 필드:
- needsRescan=true : OCR 오인식 의심 (특정 글자만 이상)
- needsRescan=false : 실제 오타/띄어쓰기 오류 (확신)
...
# ================================================================
# [4/6] 의심 자막 수집 + full-frame 재스캔
# ================================================================
echo ""
echo "[4/6] 의심 자막 재스캔"

# 의심 timeSec 수집: Claude needsRescan=true + Python 플래그 **중** 아직 이슈 없는 것
# (Claude가 이미 확신한 오타는 건드리지 않음)
python3 - <<PY > "$TMPDIR/suspects.json"
import json
with open("$FILTERED") as f: subs = json.load(f)
with open("$TMPDIR/issues1.json") as f: issues = json.load(f)

# Claude가 이미 확신(needsRescan=false) 이슈를 단 timeSec는 제외
confident_times = set()
for iss in issues:
    if iss.get("needsRescan") is False:
        idx = iss.get("index")
        if isinstance(idx, int) and 0 <= idx < len(subs):
            confident_times.add(subs[idx]["timeSec"])

suspects = set()
# Claude needsRescan=true
for iss in issues:
    if iss.get("needsRescan") is True:
        idx = iss.get("index")
        if isinstance(idx, int) and 0 <= idx < len(subs):
            suspects.add(subs[idx]["timeSec"])

# Python 플래그 있지만 Claude 확신 이슈 없는 것만 (이중 체크)
for s in subs:
    if s.get("flags") and s["timeSec"] not in confident_times:
        # 저품질 플래그 (low_confidence/top_clipped)만 재스캔 대상
...
# ================================================================
# [5/6] Claude 2차 (업데이트된/재확인된 자막만)
# ================================================================
echo ""
echo "[5/6] Claude 2차 (업데이트 재평가)"
...
echo "  → 최종 이슈 ${FINAL_COUNT}건"

# ================================================================
# [7/7] 프레임 정밀 경계 탐지 (영상 native fps + 픽셀 diff)
# ================================================================
if [[ "$FINAL_COUNT" -gt 0 ]]; then
  ...
  echo "[7/7] 프레임 정밀 경계 (native ${FPS_RATIO} = ${FPS_FLOAT}fps, ${FRAME_MS}ms/frame)"
...
PY
  ...
  python3 - <<PY > "$TMPDIR/out-refined.json"
