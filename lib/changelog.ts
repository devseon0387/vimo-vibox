...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    version: "v0.3.0",
    date: "2026-06-24",
    title: "클라이언트 검수 워크플로 대개편 + 보안·견고성 하드닝",
    description:
      "검수 뷰어 전면 재작성, 모바일/숏폼 게스트 검수, 업로드 즉시 공유 링크, 디자인 시스템 정비, 클라이언트별 코멘트 격리. 머지 전 코드리뷰로 데이터 정합·보안까지 하드닝.",
    type: "feature",
    features: [
      "검수 뷰어 전면 재작성 — 영상 아래 어두운 컨트롤 바(프레임 스텝·코멘트 네비·상태색 마커), 전체화면 몰입, 화질(원본/저화질) 토글, 미리보기→검수 확장, 개인 영상도 검수",
      "모바일·숏폼(세로) 게스트 검수 — '재생=깨끗 / 정지=전부 등장' 몰입 플레이어",
      "업로드 즉시 공유 링크 자동 생성 + 업로드 독 인라인 공유 패널, 공유 링크 영구화",
      "클라이언트 포털 — 같은 영상을 여러 고객사에 공유해도 코멘트·시청기록을 클라별로 격리",
      "디자인 시스템 — 공용 Button/Badge + 타이포 스케일 토큰, 화면당 주황 업로드 1개 원칙",
      "My box 드롭박스식 폴더 사이드메뉴(즐겨찾기 + 트리) + 스프링 모션",
    ],
    fixes: [
      "보안: 파일/폴더 이동 시 UNIQUE 충돌로 인한 disk≠DB 불일치(split-brain) 차단",
      "보안: 한글 파일명 공유 경로를 NFC 정규화로 통일 — 코멘트가 엉뚱한 파일에 붙거나 거짓 차단되던 문제 해결",
      "검수 뷰어: HLS 재생 중 '다시 시도'가 무동작이던 것 복구",
      "숏폼: 게스트 이름 늦은 도착 시 전송 버튼 영구 비활성·pagehide 리스너 누수 수정",
      "공유 미리보기 종횡비 판별 실패 시 3초 검은 화면 제거, 업로드 후 공유 링크 중복 생성 방지",
      "전역 에러 메시지 친화화(humanError), 장식 이모지 제거",
    ],
  },
  {
    id: "20260528-0.2.0",
    version: "v0.2.0",
    date: "2026-05-28",
    title: "업로더 중심 대시보드 + 두 공간 분리",
    description:
      "홈을 업로더 중심으로 재편하고 '비모 프로젝트 / My box' 두 공간을 분리. PWA 설치, AI 검수 피드백, 받은편지함, 커맨드 팔레트.",
    type: "feature",
    features: [
      "업로더 중심 대시보드 + '비모 프로젝트 / My box' 두 공간 분리",
      "PWA — 홈 화면 설치 + 모바일 최적화(파일 브라우저·공유)",
      "받은편지함(검수 대기) · ⌘K 커맨드 팔레트 · 글로벌 백그라운드 업로드 + Undo 토스트",
      "AI 검수 피드백 · 폴더 ZIP 다운/업로드 · 다중 선택 + 일괄 작업",
      "비노트(vinote) 글쓰기 컴패니언 앱 v0.1~0.3",
    ],
    fixes: [
      "보안/무결성 하드닝 P0/P1/P2(업로드·DB·세션) · personal zone 유저 간 traversal 차단",
      "미디어 직결 라우팅으로 CF(LAX) 우회 · HLS 인코딩 파이프라인",
      "데이터 저장소 SQLite → PostgreSQL(Baseon) 이전",
    ],
  },
  {
    id: "20260503-0.1.4",
